...
export default class NeutralEnding extends Phaser.Scene {
  ...
  create() {
    ...
    var mainmenu = this.add.image(document.getElementById("mainCanvas").width / 2, document.getElementById("mainCanvas").height / 2, 'neutralEnd', 0);
    ...
    let text = this.add.text(document.getElementById("mainCanvas").width / 2, document.getElementById("mainCanvas").height / 2, 'Final Neutral', { fontFamily: 'HelpMe', fontSize: 48, color: '#FF8C00' });
    text.setOrigin(0.5, 0.5)  // Colocamos el pivote en el centro de cuadro de texto 
    text.setAlign('center');

    text = this.add.text(document.getElementById("mainCanvas").width / 2, document.getElementById("mainCanvas").height / 4 * 2.5, 'Lograste construir del ataud, entieras a la convicta\n ella espera a que las desentierres, pero...\n Nunca llegas a desenterrarla.', { fontFamily: 'HelpMe' });
    text.setOrigin(0.5, 0.5)  // Colocamos el pivote en el centro de cuadro de texto 
    text.setAlign('center');
    //Título del juego
    ...
    text = this.add.text(document.getElementById("mainCanvas").width / 2, document.getElementById("mainCanvas").height / 4 * 3.5, 'Quién ha sido enterrado con la convicta?', { fontFamily: 'HelpMe', fontSize: 32, color: '#f00' });
    text.setOrigin(0.5, 0.5);  // Colocamos el pivote en el centro de cuadro de texto 
    text.setAlign('center');
